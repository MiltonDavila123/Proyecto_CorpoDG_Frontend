<template>
  ...
  <section v-if="vistaActual === 'paquetes'" class="paquetes-section">
    ...
    <div v-else class="paquetes-carousel-container">
      <!-- Flecha izquierda -->
      <button 
...
const itemsPerSlide = ref(3) // Número de tarjetas visibles
const slideWidth = computed(() => 100) // Porcentaje de desplazamiento por slide

const totalPages = computed(() => {
  if (paquetes.value.length <= itemsPerSlide.value) return 1
  return Math.ceil(paquetes.value.length / itemsPerSlide.value)
})
